...
static void duppage(u_int envid, u_int vpn) {
	...
	addr = vpt[vpn] & 0xfffff000;
	perm = vpt[vpn] & 0x00000fff;
	...
	if ((perm & PTE_D) && !(perm & PTE_LIBRARY)) {
		syscall_mem_map(0, vpn << PGSHIFT, envid, vpn << PGSHIFT, (perm | PTE_COW) & ~PTE_D);
		syscall_mem_map(0, vpn << PGSHIFT, 0, vpn << PGSHIFT, (perm | PTE_COW) & ~PTE_D);
	} else {
		syscall_mem_map(0, vpn << PGSHIFT, envid, vpn << PGSHIFT, perm);
	}
