...
void serve_debug(u_int envid, struct Fsreq_remove *rq) {
	struct File *file = get_file_absolute(rq->req_path);
...
void serve(void) {
	...
	for (;;) {
		...
		case FSREQ_DEBUG:
			serve_debug(whom, (struct Fsreq_remove *)REQVA);
			break;
...
void debug_open() {
	...
	for (int i = 0; i < MAXOPEN; i++) {
		if ((pageref(opentab[i].o_ff) > 1)) {
			...
			struct Fd *fd = (struct Fd *)opentab[i].o_ff;
